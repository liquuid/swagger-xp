...
GarageApi::GarageApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<void> GarageApi::garageInventoryGet()
{


    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/garage/inventory");
    ...
    {
        throw ApiException(400, U("GarageApi->garageInventoryGet does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("GarageApi->garageInventoryGet does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling garageInventoryGet: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling garageInventoryGet: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
